...
public class RobustProcessExecutor implements Callable<Integer> {
	...
	public RobustProcessExecutor(StructureService service) {
	...
	public void execAsync() {
		if (this.service.getDependancy() != null) {
			if (this.service.getDependancy().getResult() != ResultKarma.GOOD) {
				String msg = "Skipped, because of parent failure: " + this.service.getDependancy().getIdentifier();
	...
	public String getOutput() {
		String output = "";
		String errorStream = this.outputStreamToString(this.p.getErrorStream());
	...
	public int getReturn() throws InterruptedException, ExecutionException, TimeoutException {
		...
		long timeout = Math.max(this.timeout.getStandardSeconds(), GlobalConstants.DEF_TIMEOUT.getStandardSeconds());

		return this.future.get(timeout, TimeUnit.SECONDS);
	}

	private String outputStreamToString(InputStream os) {
